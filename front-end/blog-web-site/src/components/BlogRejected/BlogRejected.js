...
const BlogRejected = () => {
    ...
    const [uploadedImages, setUploadedImages] = useState([]); // New uploaded images
    const [existingImages, setExistingImages] = useState([]); // Existing images
    const [removedImages, setRemovedImages] = useState([]); // List of removed images
    ...
    const handleDelete = async (blogId) => {
        try {
            const response = await axios.delete(`http://localhost:5000/DeleteBlogs`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                },
                data: {
                ...
                <Modal.Body>
                    <form onSubmit={handleSubmit}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" id="blog_images" name="blog_images" className="form-control" multiple onChange={handleImageChange} />
                        </div>
